...
export function handleDrop(item) {
  ...
  if (boxOrder1.length) console.log("List 1 order:", boxOrder1);
  if (boxOrder2.length) console.log("List 2 order:", boxOrder2);
}

export function getBoxOrder(listId) {
  const list = document.getElementById(listId);
  if (!list) return [];
  return Array.from(list.children).map((item) => item.id);
}
